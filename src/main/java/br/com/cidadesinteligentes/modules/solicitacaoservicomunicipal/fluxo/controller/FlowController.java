...
public class FlowController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Flow successfully created",
                    content = @Content(schema = @Schema(implementation = FluxoRequestDTO.class))),
    ...
    ResponseEntity<?> createFlow(
        ...
        return result.hasErrors()
                ...
                : ResponseEntity.status(HttpStatus.CREATED).body(this.flowService.criarFluxo(dto));
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "FlowStep successfully created",
                    content = @Content(schema = @Schema(implementation = EtapaFluxoRequestDTO.class))),
            @ApiResponse(responseCode = "400", description = "Invalid request body",
                    content = @Content),
            @ApiResponse(responseCode = "422", description = "One or some fields are invalid", content = @Content)
    })
    ...
    ResponseEntity<?> createFlowStep(
            @RequestBody @Valid EtapaFluxoRequestDTO dto,
            BindingResult result
    ){

        return result.hasErrors()
                ? ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(ResultError.getResultErrors(result))
                : ResponseEntity.status(HttpStatus.CREATED).body(this.flowService.criarEtapaFluxo(dto));
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "List of flows retrieved",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = FluxoDadosCompletosResponseDTO.class))))
    })
    @GetMapping(value = "", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<?> getAll(){

        return ResponseEntity.status(HttpStatus.OK).body(this.flowService.buscarTodosFluxos());
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(schema = @Schema(implementation = FluxoDadosCompletosResponseDTO.class))
            )
    })
    ...
    ResponseEntity<?> getFlowById(@PathVariable("id") UUID id){
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(this.flowService.buscarFluxoPorId(id));
